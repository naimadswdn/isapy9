...
while True:
    binary = input('Please provide binary number with exactly 6 digits: ')
    if len(binary) != 6:
...
decimal = int(binary[-1])*first + int(binary[-2])*second + int(binary[-3])*third + int(binary[-4])*fourth + int(binary[-5])*fifth + int(binary[-6])*sixth
